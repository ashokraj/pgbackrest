...
;
	...
	RETVAL = factorial_iterative_c(x);
...
XS_EUPXS(XS_pgBackRest__LibC_factorial_recursive_c); /* prototype to pass -Wmissing-prototypes */
XS_EUPXS(XS_pgBackRest__LibC_factorial_recursive_c)
{
    dVAR; dXSARGS;
    if (items != 1)
       croak_xs_usage(cv,  "x");
...
;
	...
	RETVAL = factorial_recursive_c(x);
	XSprePUSH; PUSHn((double)RETVAL);
    }
    XSRETURN(1);
}
